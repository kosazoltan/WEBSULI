...
import { reviewWebTeaching, teachingText, type FetchedTeachingSource, type TeachingReview } from "./web-teaching-review";
...
export function applyTeachingPatch(html: string, raw: unknown, allowedSections?: ReadonlySet<number>): string {
  ...
  if (patch.bank) {
    ...
    for (const key of ["methods", "tasks", "quiz"] as const) for (const item of bank[key] ?? []) {
      ...
      if (!original || item.sectionIndex !== original.sectionIndex || (allowedSections && !allowedSections.has(original.sectionIndex))) throw new Error("A bankjavítás csak meglévő, érintett, azonos fejezetű tételt cserélhet.");
    }
    html = applyWebBankPatch(html, bank);
  }
...
function affectedSections(html: string, review: TeachingReview): Set<number> {
  const normalize = (s: string) => s.replace(/\s+/g, " ").trim();
  const quotes = review.issues?.flatMap(i => [i.lessonQuote, ...i.citations.filter(c => c.sourceUrl === null).map(c => c.quote)]).map(normalize) ?? [];
  const sections = elements(parse(html, { sourceCodeLocationInfo: true })).filter(n => attr(n, "data-teaching-section") !== undefined);
  const affected = new Set(sections.filter(n => {
    const location = n.sourceCodeLocation;
    return location && quotes.some(q => normalize(teachingText(html.slice(location.startOffset, location.endOffset))).includes(q));
  }).map(n => Number(attr(n, "data-teaching-section"))));
  const bank = readHtmlLessonData(html).experience;
  for (const item of [...bank.methods, ...bank.tasks, ...bank.quiz]) if (quotes.some(q => normalize(JSON.stringify(item)).includes(q))) affected.add(item.sectionIndex);
  return affected;
}
...
export async function reviewAndRepairWebTeaching(html: string, sources: FetchedTeachingSource[], options: {
  ...
  for (let attempt = 0; attempt <= 2; attempt++) {
    ...
    const allowedSections = affectedSections(html, review);
...
Kimenet JSON: {"edits":[{"sectionIndex":0,"before":"pontos meglévő HTML szövegrész","after":"teljes javított részlet"}],"bank":{"methods":[],"tasks":[],"quiz":[]}}. Egy edit csak egy meglévő data-teaching-section belsejében egyszer előforduló részletet cserélhet. Ne módosítsd a fejezet attribútumait, scripteket, stílust, navigációt, évfolyamot, banktervet vagy forráslistát. Rövid szövegcserét válassz, teljes fejezetet ne. Csak egyszerű, attribútum nélküli p,b,strong,em,i,span,br,ul,ol,li,small,sup,sub tagek engedettek. Bank opcionális: csak az érintett meglévő tétel teljes objektuma ugyanazzal az ID-val és sectionIndex-szel. Törlés vagy új tétel nincs. A bank minden válasza a javított tanításból következzen.\n${HTML_LESSON_DATA_CONTRACT}`,
      JSON.stringify({ lessonHtml: html, sources, requestedTopic: options.requestedTopic, review, allowedSectionIndices: [...allowedSections], patchFailure }), options.signal,
    );
    options.signal?.throwIfAborted();
    try { html = applyTeachingPatch(html, patch, allowedSections); patchFailure = ""; }

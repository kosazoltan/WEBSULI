...
if (!DATABASE_URL) {
  logger.error('❌ No DATABASE_URL found. Set it in .env or environment.');
  process.exit(1);
}

logger.info(`[DB] Connecting to: ${DATABASE_URL.substring(0, 40)}...`);
...
async function main() {
  ...
  try {
    logger.info('═══════════════════════════════════════════');
    logger.info('  DIRECT DATABASE DIAGNOSTIC');
    logger.info('═══════════════════════════════════════════');
    ...
    for (const col of htmlCols.rows) {
      logger.info(`  ${col.column_name}: ${col.data_type} (max: ${col.character_maximum_length || 'unlimited'}, nullable: ${col.is_nullable})`);
    }
    ...
    for (const c of constraints.rows) {
      logger.info(`  ${c.constraint_type}: ${c.constraint_name} (${c.column_name})`);
    }
    ...
    for (const t of triggers.rows) {
      logger.info(`  ${t.trigger_name}: ${t.event_manipulation} → ${t.action_statement.substring(0, 100)}`);
    }
    ...
    for (const row of improved.rows) {
      ...
      logger.info(`    status: ${row.status}, contentLength: ${row.content_length}`);
      logger.info(`    originalFileId: ${row.original_file_id}`);
      logger.info(`    preview: ${row.content_preview}`);
      logger.info('');
    }

    // 5. For EACH improved file, check the original
    logger.info('[5] Cross-check with original html_files:');
    for (const row of improved.rows) {
      ...
      if (orig.rows.length === 0) {
        logger.info(`  ❌ Original ${row.original_file_id} NOT FOUND!`);
      } else {
        const o = orig.rows[0];
        logger.info(`  Original ${o.id}: title="${o.title}", contentLength=${o.cl}`);
        logger.info(`    preview: ${o.preview}`);
        
        // Check if titles match (would indicate Apply worked)
        if (o.title === row.title) {
          logger.info(`    ✅ Titles MATCH (Apply may have worked)`);
        } else {
          logger.info(`    ❌ Titles DIFFER: original="${o.title}" vs improved="${row.title}"`);
        }
      }
    }
    ...
    if (improved.rows.length > 0 && improved.rows[0].content_length > 200) {
      ...
      logger.info(`    improved.id = ${testRow.id}`);
      logger.info(`    improved.original_file_id = ${testRow.original_file_id}`);
      logger.info(`    improved.content_length = ${testRow.content_length}`);
      ...
      logger.info(`    Actual content.length = ${improvedContent.length}`);
      ...
      logger.info(`    Original BEFORE: contentLength = ${beforeQ.rows[0]?.cl}`);
      ...
      logger.info(`    UPDATE returned = ${JSON.stringify(updateResult.rows[0])}`);
      ...
      logger.info(`    Original AFTER: contentLength = ${afterQ.rows[0]?.cl}`);

      if (afterQ.rows[0]?.cl === improvedContent.length) {
        logger.info('    ✅✅✅ UPDATE VERIFIED! Content matches!');
      } else {
        logger.info('    ❌❌❌ UPDATE FAILED! Content does NOT match!');
      }
    }

...
const schema = z.object({ ids: z.array(z.string()), answers: z.record(z.string().max(10000)), started: z.number().nonnegative(), finished: z.number().nonnegative().nullable() });
...
export function HtmlLessonTasks({ experience, material }: { experience: LessonExperience; material: string }) {
  ...
  const fresh = (size = count): Round => ({ ids: sampleTaskIds(bank, size), answers: {}, started: Date.now(), finished: null });

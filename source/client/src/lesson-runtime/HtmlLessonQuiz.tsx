...
import { quizOptionIndices, sampleIds, scoreSummary } from "@shared/lesson-experience-score";
...
export function HtmlLessonQuiz({ experience, material }: { experience: LessonExperience; material: string }) {
  ...
  return <section className="websuli-html-quiz" aria-label="Megőrzött gyakorlókvíz">
    ...
    {questions.map((q, index) => <article key={q.id} hidden={!overview && index !== Math.min(pageIndex, questions.length - 1)}><h3>{index + 1}. {q.question}</h3><div className="quiz-options">{quizOptionIndices(q).map(pick => <button key={pick} aria-pressed={round.picks[q.id] === pick} disabled={round.finished !== null || round.picks[q.id] !== undefined} onClick={() => save({ ...round, picks: { ...round.picks, [q.id]: pick } })}>{q.options[pick]}</button>)}</div>{round.picks[q.id] !== undefined && <p>{round.picks[q.id] === q.correctIndex ? "Helyes. " : "Még nem helyes. "}{q.feedbackPerOption[round.picks[q.id]]}</p>}</article>)}

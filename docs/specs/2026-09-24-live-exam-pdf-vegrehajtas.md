...
3. Új teszt: `source/tests/live-exam-pdf-2026-09-24.test.ts` a mért bemenetekkel. Meglévő tesztet NEM módosítani.
4. Kapuk: `npx tsc --noEmit`, `npx tsc -p tsconfig.test.json --noEmit`, eslint `--max-warnings 1166`, `node --import tsx --test tests/*.test.ts`, `npm run build`.
5. Ág `fix/live-exam-pdf` → PR → CI → merge → Render `/api/health` revízió + Vercel hash ellenőrzés → LEDGER.

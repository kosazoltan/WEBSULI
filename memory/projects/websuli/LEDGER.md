...
**Korlát:** felhő-munkamenet; éles DB, Render-napló és websuli.vip nem elérhető (proxy 403, nincs kulcs) → a diagnózis kódból, alügynökös cáfolással. **Az élesben mért konkrét elakadás (job 22a38c0a) oka a kimerült OpenAI-keret volt — azt a PR #126 javította;** az alábbiak a kódban maradt további elakadási pontok (élesben együtt igazolva: PR #127, done 710 s).
...
## 2026-09-25 — internetes tananyag a feltöltős Studio-gyártással (2. szelet: módszer)

**Tulajdonosi döntés:** „Csináld” — az internetes út kapja meg a feltöltős út teljes módszerét.
**Megoldás:** a webes job csak keres és letölt (`gatherWebSources`, a régi gyűjtőciklusból kiemelve). A letöltött oldalak szövegét URL-/címfejléccel `text` forrásként egylépéses Studio-futásnak adja (`startOneStepRun`, a webes workflow-n KÍVÜL: `outsideWorkflow`). A futást 3 s-onként követi (legfeljebb 120 perc), a kész leckét visszaolvassa (`readStudioLesson`: publikált lecke → html_files), `html_files` sort nem ír. A folytatás a mentett `studioRunId`-t követi. Mérethatár a tantárgy-/évfolyam-felismerő miatt: 60k/oldal, 200k összesen. Visszavonás: `WEB_RESEARCH_PIPELINE=html`.
**Verifikáció:** új egységtesztek (web-studio-handoff 5, web-research-jobs +3 valódi workflow-motorral: done html_files-írás nélkül, a Studio-futás a webes környezeten kívül indul, folytatás a mentett futással, visszaolvasás-hiba ≠ kész); Playwright webes spec 7/7 (az új Studio-eset a régi panelen bukik), 390/1440 px túlcsordulás 0.
**NOT RUN:** éles internetes gyártás ezzel a kóddal (nincs éles hálózat/kulcs a felhő-munkamenetben) — az első élő futás a helyi/éles környezetben mérendő (idő, évfolyam, forrás-URL a tudástárban).
